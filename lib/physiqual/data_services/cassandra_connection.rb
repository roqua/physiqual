  ...
  module DataServices
    class CassandraConnection
      ...
      def query_sleep(user_id, year, from, to)
        @session.execute(@query['sleep'], arguments: [user_id, year, from, to])
      end

      def query_calories(user_id, year, from, to)
        @session.execute(@query['calories'], arguments: [user_id, year, from, to])
      end

      def query_distance(user_id, year, from, to)
        @session.execute(@query['distance'], arguments: [user_id, year, from, to])
      end

      def query_steps(user_id, year, from, to)
        @session.execute(@query['steps'], arguments: [user_id, year, from, to])
      end

      def query_activities(user_id, year, from, to)
        @session.execute(@query['activities'], arguments: [user_id, year, from, to])
      end
