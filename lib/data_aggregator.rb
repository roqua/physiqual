class DataAggregator
  ...
  def steps(from, to)
    ...
    run_function(result) { |steps, results, key| [steps[key], results[key]].flatten.max if valid_result? results[key] }
  end

  def heart_rate(from, to)
    result = retrieve_data_of_all_services { |service| service.heart_rate(from, to) }
    Rails.logger.debug result
    run_function(result) { |heart_rates, results, key| [heart_rates[key], results[key]].flatten.max if valid_result? results[key]  }
  end

  def sleep(from, to)
    result = retrieve_data_of_all_services { |service| service.sleep(from, to) }
    run_function(result) { |sleep_data, results, key| [sleep_data[key], results[key]].max if valid_result? results[key]  }
  end

  def calories(from, to)
    result = retrieve_data_of_all_services { |service| service.calories(from, to) }
    run_function(result) { |calories, results, key| [calories[key], results[key]].flatten.max if valid_result? results[key]  }
  end

  def activities(from, to)
    result = retrieve_data_of_all_services { |service| service.activities(from, to) }
    run_function(result) { |_activities, results, key| results[key] if valid_result? results[key]  }
  end
